...
class Environment(tk.Tk, object):
    ...
    def build_environment(self):
        self.canvas = tk.Canvas(
            self, bg='white', height=env_height * pixels, width=env_width * pixels)
        ...
        self.goal = self.canvas.create_image(
            pixels * 3.5, pixels * 3.5, anchor='center', image=self.goal_object)

        # Create the Robot
        img_robot = Image.open("images/robot.png")
        self.robot = ImageTk.PhotoImage(img_robot)
        self.agent = self.canvas.create_image(
            pixels * 3.5, pixels * 3.5, anchor='center', image=self.robot)
    ...
    def reset(self):
        ...
        self.agent = self.canvas.create_image(
            pixels * 0.5, pixels * 0.5, anchor='center', image=self.robot)
    ...
    def final(self):
        ...
        self.initial_point = self.canvas.create_oval(
            40, 40, 60, 60, fill='red', outline='red')
